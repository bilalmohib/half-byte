...
import {
  technologiesBodySectionData,
  ITechnologiesBodySectionData,
} from "@/components/page-components/Home/TechnologiesSection/TechnologiesBodySection/data";
...
const TechnologiesBodySection = () => {
  ...
  const [selectedTechnology, setSelectedTechnology] =
    useState<ITechnologiesBodySectionData>(technologiesBodySectionData[0]);

  useLayoutEffect(() => {
    if (!techSlug) return;
    const found = technologiesBodySectionData.find((t) => t.slug === techSlug);
    if (found) setSelectedTechnology(found);
  }, [techSlug]);

  const selectTechnology = useCallback(
    (item: ITechnologiesBodySectionData) => {
      setSelectedTechnology(item);
      router.replace(technologiesHomeHref(item.slug, pathname), {
        scroll: false,
      });
    },
    [router, pathname],
  ...
  const selectedItem = dict.technologies.items[selectedTechnology.dictKey];
        ...
        <div className="col-span-1 md:col-span-3 mllg:col-span-2">
          <div className="flex flex-wrap md:flex-col gap-4 md:gap-5 lg:gap-6 py-3.75 md:py-9 lg:py-9.5">
            {technologiesBodySectionData.map((item) => {
              const itemTitle = dict.technologies.items[item.dictKey].title;
              const isSelected = selectedTechnology.slug === item.slug;
              return (
                <Button
                  key={item.slug}
                  type="button"
                  ...
                  className={cn(
                    "bg-transparent w-fit border-transparent rounded-[5px]! px-1.5! md:px-3.5! lg:px-4! h-8.25! md:h-11!",
                    isSelected
                      ? "border-black!"
                      : "border-transparent! border-b-0! border-[1.5px] shadow-none!",
                  )}
                >
                  <Paragraph
                    className={cn(
                      "md:text-xl! text-center! font-normal! text-[#4B4B4B]! leading-5! lg:leading-6.25!",
          ...
          <div className="flex flex-col gap-4.25 md:gap-6.25 lg:gap-5 py-6 md:py-10.25 lg:py-12">
            <div className="w-14 lg:w-17 h-13.25 lg:h-16.25 flex justify-center items-center bg-[#F6F6F6] rounded-[3px]">
              <Image
                ...
                width={width && width > 768 ? 39 : 32}
                height={width && width > 768 ? 40 : 32}
              />
            </div>
            <div className="flex flex-col gap-2.5 md:gap-4 lg:gap-3.75!">
              ...
              {selectedTechnology.dictKey === "grcIntelligence" ? (
                <GrcIntelligenceDetail />
              ) : (
                <Paragraph className="md:text-xl! text-[#1A1A1A]! leading-5! md:leading-5.25! lg:leading-6.25!">
...
function GrcIntelligenceDetail() {
  const dict = useT();
  const grc = dict.technologies.items.grcIntelligence;
  return (
    <div className="flex flex-col gap-2.5">
      ...
      <ul className="list-disc list-inside ps-2">
        {grc.approach.map((line, idx) => (
          <li key={idx}>
            <span className="font-roboto text-[#1A1A1A]">{line}</span>
          </li>
        ))}
      </ul>
      <Paragraph className="md:text-xl! text-[#1A1A1A]! leading-5! md:leading-5.25! lg:leading-6.25!">
