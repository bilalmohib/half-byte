...
const TechnologiesBodySection = () => {
  ...
  const selectTechnology = useCallback(
    (item: ITechnologiesBodySectionData) => {
      ...
      router.replace(technologiesHomeHref(item.slug), { scroll: false });
    },
    [router],
  );
            ...
            {technologiesBodySectionData.map((item) => (
              <Button
                ...
                className={cn(
                  ...
                  selectedTechnology?.slug === item.slug
                    ? "border-black!"
              ...
              >
                <Paragraph
                  className={cn(
                    ...
                    selectedTechnology?.slug === item.slug && "text-heading! font-medium!",
                  )}
                >
                  {item.title}
                </Paragraph>
        ...
        <div className="col-span-1 md:col-span-5 mllg:col-span-4 border-r-0 md:border-l border-t border-b-0 md:border-y-0 border-[#EAEAEA] border-solid pl-1.5 md:pl-5.25 lg:pl-15.75">
          <div className="flex flex-col gap-4.25 md:gap-6.25 lg:gap-5 py-6 md:py-10.25 lg:py-12">
            <div className="w-14 lg:w-17 h-13.25 lg:h-16.25 flex justify-center items-center bg-[#F6F6F6] rounded-[3px]">
              <Image
                src={selectedTechnology?.image as string}
                alt={selectedTechnology?.title as string}
            ...
            <div className="flex flex-col gap-2.5 md:gap-4 lg:gap-3.75!">
              ...
              <Paragraph
                className="md:text-xl! text-[#1A1A1A]! leading-5! md:leading-5.25! lg:leading-6.25!"
                dangerouslySetInnerHTML={{ __html: selectedTechnology?.description as string }}
              />
            </div>
          </div>
        </div>
