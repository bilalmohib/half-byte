    ...
    <Container>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3.75 md:gap-5.5 lg:gap-19.25">
        {ourValuesBodySectionData.map((item) => {
          const value = dict.values.items[item.dictKey];
          return (
            <div
              key={item.dictKey}
              className="flex flex-col gap-2 md:gap-3.75 rounded-[5px] border border-[#DBDBDB] border-solid bg-white py-5.25 md:py-7.75 lg:py-9 px-4.75 lg:px-8"
            >
